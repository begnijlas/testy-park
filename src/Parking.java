...
class Parking extends Thread {
	...
	private void stworzMiejsca(int odstep) {
		...
		for (int numerMiejsca = 0; numerMiejsca <= limitStanowisk - 1; numerMiejsca++) {
			...
			miejscaParkingowe[numerMiejsca] = new MiejsceParkingowe(numerMiejsca,
					wspolrzednaX + (kolumna * (30 + odstep)) + 10, wspolrzednaY + 10 + wiersz * (35 + odstep));
			kolumna++;
		}
	}
	...
	public void stworzPasek(JPanel panel) {
		...
		pasekPostepu.setBounds(wspolrzednaX, wspolrzednaY - 60, 150, 20);
		pasekPostepu.setStringPainted(true);

		iloscSamochodow.setBounds(wspolrzednaX + 160, wspolrzednaY - 55, 25, 10);
		limitSamochodow.setBounds(wspolrzednaX + 180, wspolrzednaY - 55, 35, 10);
